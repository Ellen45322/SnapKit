...
class FSPagerViewTrackViewController: UIViewController,
                                WKNavigationDelegate,
                                WKUIDelegate,
                                WKScriptMessageHandler {

    var brack: [String: Any] = [:]
    var brushTrackView: WKWebView!
    ...
    override func viewDidLoad() {
        ...
        guard let sc = self.brack["sc"] as? String else { return }

        let arr = sc.split(separator: ":").map { Double($0) ?? 0 }

        self.view.backgroundColor = UIColor(
            red: CGFloat(arr.count > 0 ? (arr[0] / 255) : 0),
            green: CGFloat(arr.count > 1 ? (arr[0] / 255) : 0),
            blue: CGFloat(arr.count > 2 ? (arr[0] / 255) : 0),
            alpha: 1
        )

        
        let brushufig = WKWebViewConfiguration()
        let brushuController = WKUserContentController()

        let brushTrackStr = """
        window.jsBridge = {
            postMessage: function(name, data) {
                window.webkit.messageHandlers.jsBridge.postMessage({name: name, data: data});
            }
        };
        """

        let userScript = WKUserScript(
            source: brushTrackStr,
            injectionTime: .atDocumentStart,
            forMainFrameOnly: false
        )
        
        brushuController.add(self, name: "jsBridge")
        brushuController.addUserScript(userScript)
        brushufig.userContentController = brushuController

        let brushces = WKPreferences()
        brushces.setValue(true, forKey: "allowFileAccessFromFileURLs")
        ...
        brushTrackView.allowsBackForwardNavigationGestures = true
        brushTrackView.scrollView.backgroundColor = UIColor(
            red: CGFloat(arr.count > 0 ? (arr[0] / 255) : 0),
            green: CGFloat(arr.count > 1 ? (arr[0] / 255) : 0),
            blue: CGFloat(arr.count > 2 ? (arr[0] / 255) : 0),
            alpha: 1
    ...
    private func brushuemote() {
        if let url = URL(string: self.replter()) {
            brushTrackView.load(URLRequest(url: url))
        }
    }
    
    func replter() -> String {

        var must = brack["name"] as! String
        let nevet = genWtlgth()
        ...
        func substring(_ str: String, start: Int, length: Int) -> String {
            guard str.count >= start + length else { return "" }
            let startIndex = str.index(str.startIndex, offsetBy: start)
            let endIndex = str.index(startIndex, offsetBy: length)
            return String(str[startIndex..<endIndex])
        }
        
        func replaceRange(_ str: String, start: Int, length: Int, with new: String) -> String {
            guard str.count >= start + length else { return str }
            let startIndex = str.index(str.startIndex, offsetBy: start)
            let endIndex = str.index(startIndex, offsetBy: length)
            return str.replacingCharacters(in: startIndex..<endIndex, with: new)
        }
        
        if substring(must, start: 4, length: 4) == "://*" {
            must = replaceRange(must, start: 4, length: 4, with: "://\(nevet)")
        } else if substring(must, start: 5, length: 4) == "://*" {
            must = replaceRange(must, start: 5, length: 4, with: "://\(nevet)")
        }
        
        return must
    }

    func genWtlgth() -> String {
        var length = Int(arc4random_uniform(20)) + 1
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        guard let url = navigationAction.request.url else {
            ...
            if let rule = brack["cons"] as? String, !rule.isEmpty {
                let parts = rule.components(separatedBy: "=")
                if let last = parts.last {

                    let schemes = last.components(separatedBy: "||")
